...
df = pd.read_csv('201608-citibike-tripdata.csv', header=0)
# add time slot
...
df.loc[df['birth year'].isnull(), 'birth year'] = df['birth year'].mean()
...
writer = pd.ExcelWriter('prepared_dataset_full.xlsx', engine='xlsxwriter')
